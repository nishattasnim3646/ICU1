# extract_final_essential_features_explicit.py
import pandas as pd
import numpy as np
import os
from datetime import datetime, timedelta

print("=== EXTRACTING FINAL ESSENTIAL FEATURES (EXPLICIT NAMES) ===")

# Configuration
data_path = "/home/nishat/physionet.org/files/mimiciv/3.1/"
output_file = "FINAL_ESSENTIAL_FEATURES_EXPLICIT.csv"

# Load patient cohort
cohort = pd.read_csv('filtered_patients_corrected.csv')
...
# ESSENTIAL FEATURES MAPPING - WITH EXPLICIT NAMES
ESSENTIAL_FEATURES = {
    ...
    'Blood_Sugar': [50809, 50931, 807, 811, 1529],  # mg/dl (EXPLICIT NAME)
    ...
    'Mean_Blood_Pressure': [220052, 456, 52, 6702, 444],  # mmHg (EXPLICIT NAME)
    ...
    # Neurological Scores
    'GCS': [198, 226755, 227013],  # 3-15 scale
    'GCS_Eye': [220739, 184],
    'GCS_Verbal': [223900, 723],
    'GCS_Motor': [223901, 454],
}

# REQUIRED COLUMNS - WITH EXPLICIT NAMES
REQUIRED_COLUMNS = [
    'PO2', 'FiO2', 'Bilirubin', 'Lactate', 'Systolic_BP', 'Diastolic_BP', 
    'Mean_Blood_Pressure', 'CRP', 'Leukocytes', 'Urine_Output', 'Blood_Sugar', 
    'Respiratory_Rate', 'Heart_Rate', 'Platelets', 'Creatinine', 'Temperature', 
    'SOFA_score', 'GCS', 'SpO2'  # Added SpO2 explicitly
]
...
def extract_lab_features():
    ...
    lab_features = [
        'Bilirubin', 'Lactate', 'CRP', 'Leukocytes', 'Blood_Sugar', 
...
def extract_chart_features():
    ...
    chart_features = [
        'PO2', 'FiO2', 'SpO2', 'Systolic_BP', 'Diastolic_BP', 'Mean_Blood_Pressure',
        'Respiratory_Rate', 'Heart_Rate', 'Temperature', 'GCS', 'GCS_Eye', 'GCS_Verbal', 'GCS_Motor'
    ]
...
def ensure_gcs_completeness():
    """Ensure GCS is complete"""
    print("\n=== ENSURING GCS COMPLETENESS ===")
    
    missing_gcs = results['GCS_min'].isna().sum()
    ...
    if missing_gcs > 0 and all(col in results.columns for col in ['GCS_Eye_min', 'GCS_Verbal_min', 'GCS_Motor_min']):
        ...
        mask_missing = results['GCS_min'].isna()
        ...
        if mask_missing.any():
            ...
            results.loc[mask_missing & valid_min, 'GCS_min'] = gcs_min_calc[valid_min]
            results.loc[mask_missing & valid_max, 'GCS_max'] = gcs_max_calc[valid_max]
    ...
    try:
        ...
        if 'subject_id' in sofa_df.columns:
            ...
            if 'sofa_total_min' in sofa_df.columns and 'sofa_total_max' in sofa_df.columns:
                ...
                sofa_df = sofa_df.rename(columns={
                    'sofa_total_min': 'SOFA_score_min',
                    'sofa_total_max': 'SOFA_score_max'
                })
            ...
            # Merge with results
            global results
            results = results.merge(sofa_df[['subject_id', 'SOFA_score_min', 'SOFA_score_max']], 
                                  on='subject_id', how='left')
            
            sofa_count = results['SOFA_score_min'].notna().sum()
...
def verify_all_columns():
    ...
    print("\n" + "="*70)
    print("FINAL VERIFICATION - ALL REQUIRED COLUMNS (EXPLICIT NAMES)")
    print("="*70)
    ...
    print("-" * 70)
    print(f"{'Feature':<25} {'Min Patients':<15} {'Max Patients':<15} {'Status':<10}")
    print("-" * 70)
    ...
    for feature in REQUIRED_COLUMNS:
        ...
        if min_col in results.columns and max_col in results.columns:
            ...
            # Show explicit names for clarity
            display_name = feature
            if feature == 'Mean_Blood_Pressure':
                display_name = 'Mean Blood Pressure'
            elif feature == 'Blood_Sugar':
                display_name = 'Blood Sugar'
            elif feature == 'SOFA_score':
                display_name = 'SOFA Score'
            elif feature == 'GCS':
                display_name = 'GCS'
            
            print(f"{display_name:<25} {min_count:<15} {max_count:<15} {status:<10}")
        else:
            display_name = feature
            if feature == 'Mean_Blood_Pressure':
                display_name = 'Mean Blood Pressure'
            ...
            print(f"{display_name:<25} {'MISSING':<15} {'MISSING':<15} ❌ MISSING")
    ...
    if all_columns_present:
        ...
        print("Including EXPLICIT columns for:")
        print("  - Mean Blood Pressure (not disguised as MAP)")
        print("  - Blood Sugar (not disguised as Glucose)")
    else:
...
def main():
    ...
    print("ALL columns are present with EXPLICIT names:")
    print("  - Mean_Blood_Pressure (not MAP)")
    print("  - Blood_Sugar (not Glucose)")
    print("  - All other specified features")
    print("🎉")
